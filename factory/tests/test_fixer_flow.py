...
class TestHandleFeaturePushAfterFix:
    def test_post_fix_push_launches_test_after_fix_once(self) -> None:
        ...
        pr_list = [{"number": 10}]
        fix_marker = role_marker("fix", 10)
        test_after_fix_marker = role_marker("test-after-fix", 10)
        ...
        assert args[2] == "test"
        assert kwargs["marker"] == test_after_fix_marker
        assert "test-after-fix" in kwargs["idempotency_key"]

    def test_post_fix_push_skips_when_test_after_fix_already_launched(self) -> None:
        event = {"ref": "refs/heads/feature/10-something"}
        pr_list = [{"number": 10}]
        fix_marker = role_marker("fix", 10)
        test_after_fix_marker = role_marker("test-after-fix", 10)
        ...
        def has_marker(_repo: str, _num: int, marker: str) -> bool:
            return marker in {fix_marker, test_after_fix_marker}
