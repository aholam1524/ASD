...
class TestIssueNumberFromBranch:
    def test_feature_branch(self) -> None:
        assert issue_number_from_branch("feature/58-set-factory-status") == 58
...
class TestIssueNumberFromPrBody:
    def test_closes_line(self) -> None:
        assert issue_number_from_pr_body("Closes #58\n") == 58
...
class TestSetFactoryStatus:
    def test_replaces_other_factory_labels(self) -> None:
        ...
        set_factory_status("o/r", 58, "factory-review", gh_json=gh_json, run=run)

        gh_json.assert_called_once()
        args = run.call_args[0][0]
        assert args[:4] == ["gh", "issue", "edit", "58"]
    ...
    def test_dev_launch_sets_factory_dev_on_issue(self) -> None:
        with patch.object(dispatch, "comment_has_marker", return_value=False), patch.object(
            ...
            result = launch_role(
                ...
                title="Title",
                body="Body",
                html_url="https://github.com/o/r/issues/58",
                is_pr=False,
            )

        assert result == 0
        status_mock.assert_called_once_with("o/r", 58, "factory-dev")

    def test_review_launch_sets_factory_review_on_issue(self) -> None:
        with patch.object(dispatch, "comment_has_marker", return_value=False), patch.object(
            dispatch, "launch_agent", return_value=("agent-1", "run-1")
        ), patch.object(dispatch, "post_comment"), patch.object(
            status_labels, "resolve_issue_number_for_pr", return_value=58
        ...
        assert result == 0
        status_mock.assert_called_once_with("o/r", 58, "factory-review")
    ...
    def test_test_on_main_pr_uses_waiting_main(self) -> None:
        with patch.object(
            ...
            apply_factory_status_for_launch(
                ...
                number=99,
                is_pr=True,
                factory_issue_number=58,
            )
        status_mock.assert_called_once_with("o/r", 58, "factory-waiting-main")
